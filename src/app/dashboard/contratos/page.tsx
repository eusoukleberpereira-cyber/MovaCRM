...
export default function ContratosPage() {
  ...
  function openCreate() {
    reset({ data_inicio: format(new Date(), "yyyy-MM-dd"), data_vencimento: "", valor_mensal: undefined as unknown as number })
    setModalOpen(true)
  }
    ...
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-background">
              ...
              <th className="px-4 py-3 text-left font-medium text-muted">Valor/mês</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-10 text-center text-muted">Nenhum contrato cadastrado.</td></tr>
            ) : contratos.map((c, i) => {
              const vencido = c.status === "ativo" && isPast(parseISO(c.data_vencimento))
              return (
                <tr key={c.id} className={i % 2 !== 0 ? "bg-background/50" : ""}>
                  ...
                  <td className={`px-4 py-3 font-mono text-xs ${vencido ? "text-danger font-bold" : "text-text"}`}>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Novo Contrato">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Input label="Valor mensal (R$)" type="number" step="0.01" placeholder="0.00"
